import { ReviewInfo1, ReviewInfo2_average, ReviewInfo2_score, ReviewInfo2_myGender, ReviewInfo2_partnerGender, ReviewInfo3 } from "~/utils/interface";
...
export const setReviewInfo2_average = (reviewInfo2_average: ReviewInfo2_average) => {
  return {
...
export const setReviewInfo2_myGender = (reviewInfo2_myGender: ReviewInfo2_myGender) => {
  return {
...
export const setReviewInfo2_partnerGender = (reviewInfo2_partnerGender: ReviewInfo2_partnerGender) => {
  return {
...
const ReviewUploadReducer = (state = initialState, action) => {
  switch (action.type) {
    case SET_REVIEW_INFO1:
      const obj1 = state.reviewInfo1.find((item) => item.productId === action.reviewInfo1.productId);
      if (obj1) {
        return {
          ...state,
          reviewInfo1: [...state.reviewInfo1.filter((item) => item.productId !== action.reviewInfo1.productId), action.reviewInfo1]
        }
      } else {
        return {
          ...state,
    ...
    case SET_REVIEW_INFO2_AVERAGE:
      const obj2 = state.reviewInfo2_average.find((item) => item.productId === action.reviewInfo2_average.productId);
      if (obj2) {
        return {
          ...state,
          reviewInfo2_average: [...state.reviewInfo2_average.filter((item) => item.productId !== action.reviewInfo2_average.productId), action.reviewInfo2_average]
        }
      } else {
        return {
          ...state,
          reviewInfo2_average: [...state.reviewInfo2_average, action.reviewInfo2_average]
        }
      }

    case SET_REVIEW_INFO2_SCORE:
      const obj3 = state.reviewInfo2_score.find((item) => item.productId === action.reviewInfo2_score.productId);
      if (obj3) {
        return {
          ...state,
          reviewInfo2_score: [...state.reviewInfo2_score.filter((item) => item.productId !== action.reviewInfo2_score.productId), action.reviewInfo2_score]
        }
      } else {
        return {
          ...state,
          reviewInfo2_score: [...state.reviewInfo2_score, action.reviewInfo2_score]
        }
      }

      case SET_REVIEW_INFO2_MYGENDER:
        const obj4 = state.reviewInfo2_myGender.find((item) => item.productId === action.reviewInfo2_myGender.productId);
        if (obj4) {
          return {
            ...state,
            reviewInfo2_myGender: [...state.reviewInfo2_myGender.filter((item) => item.productId !== action.reviewInfo2_myGender.productId), action.reviewInfo2_myGender]
          }
        } else {
          return {
            ...state,
            reviewInfo2_myGender: [...state.reviewInfo2_myGender, action.reviewInfo2_myGender]
          }
        }
        case SET_REVIEW_INFO2_PARTNERGENDER:
          const obj5 = state.reviewInfo2_partnerGender.find((item) => item.productId === action.reviewInfo2_partnerGender.productId);
          if (obj5) {
            return {
              ...state,
              reviewInfo2_partnerGender: [...state.reviewInfo2_partnerGender.filter((item) => item.productId !== action.reviewInfo2_partnerGender.productId), action.reviewInfo2_partnerGender]
            }
          } else {
            return {
              ...state,
              reviewInfo2_partnerGender: [...state.reviewInfo2_partnerGender, action.reviewInfo2_partnerGender]
            }
          }

    case SET_REVIEW_INFO3:
      const obj6 = state.reviewInfo3.find((item) => item.productId === action.reviewInfo3.productId);
      if (obj6) {
        return {
          ...state,
          reviewInfo3: [...state.reviewInfo3.filter((item) => item.productId !== action.reviewInfo3.productId), action.reviewInfo3]
        }
      } else {
        return {
          ...state,
